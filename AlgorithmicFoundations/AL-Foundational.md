...
容量を倍ずつ増やすとすると、例えば$$n = 5$$のとき、2個目の要素を追加する際にダブリングが生じてすでに存在する1個の要素をコピーする必要があり、次は3個目の要素を追加する際にすでに存在する2個の要素をコピーする必要があり、その次は5個目の要素を追加する際にすでに存在する4個の要素をコピーする必要があるので、合計で$$1 + 2 + 4 = 7$$回のコピーが必要になる。つまり、合計で$$n \text{よりも大きい最小の2の累乗数} - 1$$回のコピーが発生する。そのため１回あたりのコピーにかかる平均時間は$$O(1)$$であり、償却定数時間になる。このように、配列の容量を倍ずつ増やすことをダブリング（doubling）と呼ぶ。
