...
各ノードの値が子ノードの値より常に等しいか小さい（または大きい）という制約を持つ木構造である。優先度付きキューや、プリム法やダイクストラ法などのグラフ問題のアルゴリズムでも使われている。単にヒープといった場合、通常は二分ヒープを指す。二分ヒープは二分木を使って作られるヒープであり、ヒープの制約に加えて、バランス木である必要がある。
