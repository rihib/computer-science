...
挿入ソートは、自分の値とそれより前の値を比較して、適切な位置に挿入していくアルゴリズムである。最悪計算量は$$O(n^2)$$になるが、ソート対象がほとんどソートされている場合は内側のループを回る回数が少なくて済むので、時間計算量は$$O(n)$$に近づく。実際のデータはほとんどソートされている場合が多く、その場合に挿入ソートは高速に動作するため、実用的なアルゴリズムである。
...
平均して$$O(n \log n)$$の時間計算量でソートを行う。後述するヒープソートやマージソートも$$O(n \log n)$$だが、それらに比べて基本操作が簡単なので、実際の計算時間は少なくなる。多くの問題に対して、最高速のソートアルゴリズムであると言われている。欠点は、最悪の場合に性能が極端に悪くなる（$$O(n^2)$$の時間計算量になる）ことである。実際にソートを必要とする問題では、クイックソートの適用をまず考えるべきである。

クイックソートは、再帰をしていく中で短い範囲のデータを繰り返しみていく（深さ優先探索になる）ため、キャッシュの効果を受けやすい。対してマージソートは全体を見てマージしていく必要があるため、キャッシュの効果を受けにくい。そのため、クイックソートはマージソートに比べてメモリ局所性が高いので、オーダーが同じでもクイックソートの方が速くなると言われている。
