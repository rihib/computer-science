...
どのノードにおいても左右の部分木の高さの差が1以下であるような二分探索木をAVL木と呼ぶ。AVL木は挿入や削除の際に木の再構築を行うことでバランスを保つ。二分探索木であることに変わりはないので、探索についてはすでに前述したとおりである。そのためAVL木の制約により、探索の最悪計算量は$$O(\log n)$$に収まる。
