...
隣接行列表現の場合は行列の乗算をすることができる。一般的に、隣接行列を$$n$$乗することで、グラフ内の各頂点間の長さ$$n$$のパスの数を得ることができる。例えばある非重み付きグラフの隣接行列を２乗すると、2歩で到達できる頂点の数を表す行列が得られる。

（行列の乗算のやり方については[行列の積（掛け算）の求め方をわかりやすく解説します！](https://lab-brains.as-1.co.jp/enjoy-learn/2023/07/50258/)を参照）
