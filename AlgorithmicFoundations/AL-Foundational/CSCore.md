...
部分順序付き木は、プライオリティキューを実現する１つの選択肢にはなるが、まだ複雑すぎる。特にポインターを使って木を表現するために余分の記憶域を必要とし、またルートを取り除いた後の組み替え操作を木の左右のバランスを考慮せずに行うので、バランスを崩し、挿入や削除の計算量が増えてしまう可能性がある。そのため、プライオリティキューを実装する際はヒープ（後述）がよく使われる。
...
どのノードにおいても左右の部分木の高さの差が1以下であるような二分探索木をAVL木と呼ぶ。AVL木は最初に考案された平衡二分探索木で、挿入や削除の際に木の再構築を行うことでバランスを保つ。二分探索木であることに変わりはないので、探索についてはすでに前述したとおりである。そのためAVL木の制約により、探索の最悪計算量は$$O(\log n)$$に収まる。

挿入や削除の際に制約を満たすために木の回転を行うが、この回転に要する時間は$$O(1)$$であるため、木の高さは$$O(\log n)$$であることから、挿入や削除の計算量は$$0(\log n)$$に収まる。
...
このことから、AVL木は探索に要する計算量が平均的にも最悪の場合にも普通の二分探索木より少なく、また挿入や削除には余分な手間がかかるが、計算量のオーダーは変わらないため、十分実用的なアルゴリズムだと考えられる。特に挿入や削除に比べて探索の回数が多い場合に特に適しているアルゴリズムだと言える。挿入や削除が多い場合には単純な二分探索木とどちらを採用すべきかは一概には決められない。理論的な観点からは、探索と挿入を同時に$$O(\log n)$$の計算量で処理できることを初めて示した点でAVL木の価値は大きい。

[LeetCode - Balanced Binary Tree](https://github.com/rihib/leetcode/pull/30)

#### 赤黒木

赤黒木（Red-Black Tree）は以下の5つの性質を持つことで、木のバランスを保っている。これらの性質により、赤黒木は常におおよそ平衡な状態を保ち、探索、挿入、削除の各操作を最悪の場合でも$$O(\log n)$$の時間で実行できるようにしている。挿入や削除の際には、必要に応じて木の部分的な回転やノードの色の変更が行われ、赤黒木の性質が保たれるよう調整される。

1. 各ノードは赤か黒のいずれかの色を持つ
2. ルートノードは常に黒である
3. すべてのリーフノード（NILノード）は黒である
4. 赤ノードの子ノードは必ず黒である（赤の連続が許されない）
5. 各ノードからその子孫の葉ノードまでのすべてのパスには、同じ数の黒ノードが存在する

#### ヒープ

各ノードの値が子ノードの値より常に等しいか小さい（または大きい）という制約を持つ木構造である。プライオリティキューや、プリム法やダイクストラ法などのグラフ問題のアルゴリズムでも使われている。単にヒープといった場合、通常は二分ヒープを指す。二分ヒープは二分木を使って作られるヒープであり、ヒープの制約に加えて、バランス木である必要がある。
