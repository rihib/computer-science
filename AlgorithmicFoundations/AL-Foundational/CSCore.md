...
探索操作を必要とせず、最小値（または最大値）を取り出す操作と挿入、削除のみを行うデータ構造をプライオリティキュー（優先順位付き待ち行列）と呼ぶ。最小値（または最大値）を取り出す操作は$$O(1)$$で、挿入と削除は$$O(\log n)$$で実行できる。プライオリティキューは平衡木やB木でも実現できるが、これらを用いるのはやや大袈裟で、通常は探索の必要がないのであれば部分順序付き木やヒープなどで実現できる。特にヒープベースプライオリティキューがよく使われる。
...
同じ二分木であっても、ノードの間に成立する大小関係の式を変えることによって、最大値（または最小値）だけを効率よく求めるデータ構造が得られる。最大値（または最小値）をルートに置き、各ノードの値がその左右のどちらの子ノードの値よりも小さく（または大きく）ならないようにすることで、最大値（または最小値）を求める操作を$$O(1)$$で行うことができる。このようなデータ構造を部分順序付き木（partially ordered tree）と呼ぶ。
...
| アルゴリズム | 探索（平均） |  探索（最善） | 探索（最悪） | 挿入 | 削除 | 空間計算量 | Pros | Cons |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AVL木 | $$O(\log n)$$ | $$O(\log n)$$ | $$O(\log n)$$ | $$O(\log n)$$ | $$O(\log n)$$ | $$O(n)$$ | 探索は平均・最悪の場合の両方で二分探索木や赤黒木よりも速い | 挿入や削除は遅い |
| 赤黒木 | $$O(\log n)$$ | $$O(\log n)$$ | $$O(\log n)$$ | $$O(\log n)$$ | $$O(\log n)$$ | $$O(n)$$ | AVL木より挿入や削除が速い | 探索はAVL木より遅い |
| ヒープ | - | - | - | $$O(\log n)$$ | $$O(\log n)$$ | $$O(n)$$ | 最大値（または最小値）の取り出しが$$O(1)$$ | 探索はできない |

#### AVL木

どのノードにおいても左右の部分木の高さの差が1以下であるような二分探索木をAVL木と呼ぶ。AVL木は最初に考案された平衡二分探索木で、挿入や削除の際に木の再構築を行うことでバランスを保つ。二分探索木であることに変わりはないので、探索についてはすでに前述したとおりである。そのためAVL木の制約により、探索の最悪計算量は$$O(\log n)$$に収まる。挿入や削除の際に制約を満たすために木の回転を行うが、この回転に要する時間は$$O(1)$$であるため、木の高さは$$O(\log n)$$であることから、挿入や削除の計算量は$$0(\log n)$$に収まる。理論的な観点からは、探索と挿入を同時に$$O(\log n)$$の計算量で処理できることを初めて示した点でAVL木の価値は大きい。
...
赤黒木はAVL木よりも平衡性についての条件が緩いため、探索は遅くなるが、挿入や削除は速くなる。赤黒木は常におおよそ平衡な状態を保ち、探索、挿入、削除の各操作を最悪の場合でも$$O(\log n)$$の時間で実行できるそのため、リアルタイムシステムのような時間センシティブなアプリケーションにおいて価値があるのと、最悪のケースを保証する他のデータ構造における価値ある部品となっている。例えば、計算幾何学で用いられる多くのデータ構造は最悪のケースでの計算量を保証する必要のあるため赤黒木をベースとしているし、LinuxカーネルのCFS（Completely Fair Scheduler）も赤黒木を使用している。

[赤黒木](https://ja.wikipedia.org/wiki/赤黒木)

#### ヒープ

ヒープは部分順序付き木を配列上に表現したデータ構造であり、ノード同士の親子関係をポインタを用いずに、要素の添字の間の算術的な関係によって表す点が大きな特徴である。人によってはヒープという言葉を一般の部分順序付き木を指すのに使うこともある。ヒープは配列を用いて表現されるという特性上、常に完全二分木になる。最大値（または最小値）を取り除いた後、末尾の要素を先頭に持ってきて、部分順序付き木の条件を満たすように親子を入れ替える操作を行う。挿入の場合は、要素を末尾に追加し、同様に条件を満たすように親子を入れ替える。
...
| アルゴリズム | 最悪計算量 | 平均計算量 | 最良計算量 | 空間計算量 | 最悪空間計算量 | 安定性 | インプレース | Pros | Cons |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 挿入ソート | $$O(n^2)$$ | - | $$O(n)$$ | $$O(1)$$ | $$O(1)$$ | 安定 | YES | ソート済み・小さいデータセットに対して高速 | ランダム・大きいデータセットに対して遅い |
| 選択ソート | $$O(n^2)$$ | $$O(n^2)$$ | $$O(n^2)$$ | $$O(1)$$ | $$O(1)$$ | 不安定 | YES | メモリ使用量が少ない | 遅い |
| クイックソート | $$O(n^2)$$ | $$O(n \log n)$$ | $$O(n \log n)$$ | $$O(\log n)$$ | $$O(n)$$ | 不安定 | YES | 多くの問題に対して最高速 | 最悪計算量が$$O(n^2)$$になる可能性がある |
| マージソート | $$O(n \log n)$$ | $$O(n \log n)$$ | $$O(n \log n)$$ | $$O(n)$$ | $$O(n)$$ | 安定 | NO | 外部記憶装置上のデータ・連結リストのソートに適している | 必要な作業用の記憶域が多い |
| ティムソート | $$O(n \log n)$$ | $$O(n \log n)$$ | $$O(n)$$ | $$O(n)$$ | $$O(n)$$ | 安定 | NO | 実際のデータに対して効率的 | ランダムなデータに対してはクイックソートより遅い |
| ヒープソート | $$O(n \log n)$$ | $$O(n \log n)$$ | $$O(n \log n)$$ | $$O(1)$$ | $$O(1)$$ | 不安定 | YES | ヒープ構造を使っているため、データの追加や削除が容易 | メモリ局所性が悪い |
...
時間計算量は$$O(n \log n)$$で、マージソートと同様に最悪の場合でもオーダーが変わらない安全なアルゴリズムだが、実際には平均的な計算速度ではクイックソートに劣る。マージソートにもやや劣る。マージソートは選択ソートの改良版と捉えることができる。選択ソートでは単純に最大値（または最小値）を見つけるのに配列を線形探索する必要があり、この部分で$$O(n)$$かかるために全体では$$O(n^2)$$になる。しかし、プライオリティキューを使って$$O(\log n)$$の時間計算量で最大値（または最小値）を取り出すことができれば、$$0(n \log n)$$の計算量でソートができる。これがヒープソートである。
